...
import fpinscala.datastructures.List.{dropWhile, dropWhile2}
...
object MyModule {
  ...
  def main(args: Array[String]): Unit = {
    val nList = fpinscala.datastructures.List(1, 2, 3, 4, 5)

    println(dropWhile(nList, (x: Int) => x > 2))
    println(dropWhile2(nList)(x => x > 2))
  }
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    def loop(n: Int, isGood: Boolean): Boolean = {
      if(n >= as.length - 1 || !isGood) isGood
      else loop(n+1, gt(as(n), as(n+1)) && isGood)
    }
    loop(0, isGood = true)
  }

  def cInt(a: Int, b: Int): Boolean = {
    if(a <= b) true
    else false
  }
  def main(args: Array[String]): Unit = {
    println(isSorted(Array(1,2,3,4,4), cInt))
